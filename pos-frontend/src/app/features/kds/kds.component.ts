...
@Component({
  ...
  styles: [`
    ...
    .actions button { width: 100%; padding: .5rem; border: none; border-radius: 4px; cursor: pointer; background: #2980b9; color: #fff; }
...
export class KdsComponent implements OnInit, OnDestroy {
  ...
  loadItems(): void {
    this.http.get<ItemPedidoDTO[]>(`${environment.apiUrl}/api/v1/kds/items`)
      .subscribe(items => this.items = items);
  }

  updateEstado(id: string, estado: string): void {
    this.http.put(`${environment.apiUrl}/api/v1/kds/items/${id}/estado`, { estado })
